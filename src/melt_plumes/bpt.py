...
def bpt(
    ...
    **melt_kwargs,
    ...
    m, T_b, S_b = melt_rate(-z, T_p, S_p, w, Γ_T=Γ_T, Γ_S=Γ_S, C_d=C_d, **melt_kwargs)
    ...
    dfluxes[2] = (
        α * w * T_o + m * T_b - C_d**0.5 * w * Γ_T * (T_p - T_b)
    )  # Temperature
    dfluxes[3] = α * w * S_o + m * S_b - C_d**0.5 * w * Γ_S * (S_p - S_b)  # Salinity
    return dfluxes
