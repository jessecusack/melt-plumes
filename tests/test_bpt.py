...
def test_plume_homogeneous_ocean_without_melt() -> None:
    ...
    pi = pres(-zi, lat)  # Pressure (dbar)
    Ti = fp(Si, pi)  # Temperature is the freezing point (C)
    ρi = pden(S_o, T_o, pi, pr=0)  # Far-field ocean density (kg m-3)
    ρ_oi = pden(Si, Ti, pi, pr=0)  # Plume density (kg m-3)
